...
export function getSystemPrompt(currentHtml?: string, temporalContext?: TemporalContext): string {
  const isFirstGeneration = !currentHtml;
...
${buildTemporalBlock(temporalContext)}${buildFirstGenerationBlock(isFirstGeneration)}${buildCurrentWebsiteBlock(currentHtml)}${buildEditModeBlock(currentHtml)}
