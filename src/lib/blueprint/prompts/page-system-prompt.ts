...
export function getPageSystemPrompt(
  ...
  const sectionsList = page.sections
    .map((s, i) => {
      const meta: string[] = [];
      if (s.sectionType && s.sectionType !== 'custom') meta.push(`type:${s.sectionType}`);
      if (s.layoutHint && s.layoutHint !== 'stacked') meta.push(`layout:${s.layoutHint}`);
      if (s.itemCount) meta.push(`${s.itemCount} items`);
  ...
  const siteFactsBlock = blueprint.siteFacts
    ? `<site_facts>
These are verified business details provided by the business owner. Use them EXACTLY as given for address, phone, hours, social links, etc. Do NOT invent, guess, or override any details listed here.
...
Typography: Use the design system fonts with variety in weight, size, and spacing. Create dramatic scale differences between headings and body. Vary font-weight (300/400/500/600/700) meaningfully across elements.
Color: Dominant colors with sharp accents outperform timid, evenly-distributed palettes. The design system gives you the palette — your job is how aggressively and creatively you apply it.
Backgrounds: Create atmosphere — layer CSS gradients using your design tokens, use geometric SVG patterns, add contextual texture. NEVER default to plain white/gray backgrounds.
...
The best designs feel inevitable in hindsight but surprising at first glance.
</creative_direction>
...
<creative_seed>
Design mood: "${designSystem.mood}"
Surface treatment: ${designSystem.surfaceTreatment || 'clean'}
Your job is to EXPRESS this mood through layout choices, spacing rhythm, visual weight, and micro-interactions — not just through colors (which are already set). The mood should be felt in how elements are arranged, how content breathes, and how interactions feel. Apply the surface treatment across section backgrounds.
...
${[
  contentStrategy.valuePropositions?.length
    ? `Value Props: ${contentStrategy.valuePropositions.join(' | ')}` : '',
  contentStrategy.differentiators?.length
    ? `Differentiators: ${contentStrategy.differentiators.join(' | ')}` : '',
  contentStrategy.keyStats?.length
